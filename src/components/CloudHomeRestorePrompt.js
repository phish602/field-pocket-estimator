  ...
  useEffect(() => {
    ...
    Promise.resolve(executeSupabaseCloudRestore({
      ...
      .then((result) => {
        ...
        refreshCloudStatusRef.current();
      })
      .catch(() => {
        if (mountedRef.current) setRestoreResult({ status: CLOUD_RESTORE_STATUS.ERROR, error: "Restore could not be completed on this device." });
      })
